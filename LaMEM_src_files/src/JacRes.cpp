...
PetscErrorCode JacResFormResidual(JacRes *jr, Vec x, Vec f)
{
    ...
    PetscViewer viewer;
    PetscBool flg;
    char name_rhs[_str_len_];

    ierr = PetscOptionsHasName(NULL, NULL, "-dump_rhs", &flg); CHKERRQ(ierr);

    if (flg)
    {
    // dump residual vec
    sprintf(name_rhs,"Matrices/rhs.bin");
	PetscPrintf(PETSC_COMM_WORLD, "==== !!!!! ==== dump residual ==== !!!!! ====\n");
    PetscViewerBinaryOpen(PETSC_COMM_WORLD,name_rhs,FILE_MODE_WRITE,&viewer);
    VecView(f, viewer);
...
PetscErrorCode JacResGetResidual(JacRes *jr)
{
    ...
    //PetscInt n_rho = (nx*ny*nz)+((nx+1)*ny*nz)+(nx*ny*(nz+1))+(nx*(ny+1)*nz);   // length of rho vector
	PetscInt n_rho = (nx*ny*nz);   // length of rho vector
	PetscInt num = 0;
    Vec rho_vec;
	ierr = VecCreateSeq(PETSC_COMM_SELF,n_rho,&rho_vec); CHKERRQ(ierr);
	ierr = VecSet(rho_vec,num);  CHKERRQ(ierr);
	VecAssemblyBegin(rho_vec);
    VecAssemblyEnd(rho_vec); 
	...
	START_STD_LOOP
	{
		...
		// store current rho value in vector
        ierr = VecSetValue(rho_vec,iter-1,rho,INSERT_VALUES);   CHKERRQ(ierr);
		...
		if (fssa_allVel){
			...
			fz[k][j][i] -= (szz + (vz[k][j][i])*tz)/bdz + gz/2.0;  
			if (k != 31){
			 fz[k+1][j][i] += (szz + (vz[k+1][j][i])*tz)/fdz - gz/2.0;}
		}
	...
	PetscViewer viewer;
	PetscBool   flg;
	char        name[100], name_vec[20];

    VecAssemblyBegin(rho_vec);
    VecAssemblyEnd(rho_vec); 

	ierr = PetscOptionsHasName(NULL, NULL, "-dump_rho", &flg); CHKERRQ(ierr);

	if (flg)
	{

		// dump rho vector
		sprintf(name_vec,"Matrices/rho.bin");
		PetscPrintf(PETSC_COMM_WORLD, "==== !!!!! ==== rho vec ==== !!!!! ====\n");
		PetscViewerBinaryOpen(PETSC_COMM_WORLD,name_vec,FILE_MODE_WRITE,&viewer);
		VecView(rho_vec, viewer);
		PetscViewerDestroy(&viewer);
