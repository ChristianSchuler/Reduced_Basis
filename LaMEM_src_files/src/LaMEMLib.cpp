...
PetscErrorCode LaMEMLibSolve(LaMEMLib *lm, void *param)
{
	...
	//==============
        ...
        PetscBool flg1;
		PetscBool flg2;
        ierr = PetscOptionsHasName(NULL, NULL, "-only_matrix", &flg1); CHKERRQ(ierr);
		ierr = PetscOptionsHasName(NULL, NULL, "-dump_decomposition_matrices", &flg2); CHKERRQ(ierr);
        if (flg1 || flg2)
	    {
			// switch flag
		lm->jr.ctrl.initGuess = 0;  // no initial guess if only matrices should be dumped
		}
	...
	while(!TSSolIsDone(&lm->ts))
	{
	    ...
	    ierr = PMatMonoAssemble(pm); CHKERRQ(ierr);

         // get size of rhs
		PetscInt size;
		VecGetSize(lm->jr.gsol,&size);

		Vec vec1;
		PetscInt  num = 0;
		ierr = VecCreateSeq(PETSC_COMM_SELF,size,&vec1); CHKERRQ(ierr);
		ierr = VecSet(vec1,num);  CHKERRQ(ierr);
		VecAssemblyBegin(vec1);
        VecAssemblyEnd(vec1); 

        // calculate residual
		ierr = JacResFormResidual(&lm->jr, vec1, lm->jr.gres); CHKERRQ(ierr);
        ...
        ierr = PetscOptionsHasName(NULL, NULL, "-dump_sol", &flg); CHKERRQ(ierr);

        if (flg)
        {
        sprintf(name_rhs,"Matrices/sol.bin");
        PetscViewerBinaryOpen(PETSC_COMM_WORLD,name_rhs,FILE_MODE_WRITE,&viewer);
        VecView(lm->jr.gsol, viewer);
   ...
   /* PetscViewer viewer;
    ...
    char name[100], name_rhs[_str_len_];

    ierr = PetscOptionsHasName(NULL, NULL, "-dump_rhs_zero", &flg); CHKERRQ(ierr);

    if (flg)
    {

	//========================dump rhs of final equation ========================================
	    // to do that assume that the solution vector is a zero vector and then create residual
		// --> residual is then simply the rhs vector
        // get size of rhs
		PetscInt size;
		VecGetSize(lm->jr.gsol,&size);
