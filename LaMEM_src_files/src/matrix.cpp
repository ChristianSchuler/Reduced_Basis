...
PetscErrorCode PMatMonoAssemble(PMat pm)
{
	...
	PetscInt    iter, i, j, k, nx, ny, nz, sx, sy, sz, rescal, neta;
	...
	PetscInt    pdofidx[7];
	PetscScalar cf[7];
	PetscScalar v_fix[36];
	PetscInt    n_vis,n_p,n_tot;
	...
	n_vis = ((nx+1)*ny*nz)+(nx*(ny+1)*nz)+(nx*ny*(nz+1));  // dimension of decomposition matrices (second velocity derivative block)
    n_p   = nx*ny*nz;
	n_tot = n_vis + n_p;
    // initialize eta vector
    PetscInt n_eta = (nx*ny*nz)+((nx+1)*(ny+1)*nz)+((nx+1)*ny*(nz+1))+(nx*(ny+1)*(nz+1));   // length of eta vector
	PetscScalar num = 0;
    Vec eta_vec;
	//ierr = VecCreateSeq(PETSC_COMM_SELF,n_eta,&eta_vec); CHKERRQ(ierr);
	//ierr = VecSet(eta_vec,num);  CHKERRQ(ierr);

    VecCreate(PETSC_COMM_WORLD,&eta_vec);
    VecSetSizes(eta_vec,PETSC_DECIDE,n_eta);
    VecSetFromOptions(eta_vec);
    ierr = VecSet(eta_vec,num);  CHKERRQ(ierr);

	VecAssemblyBegin(eta_vec);   CHKERRQ(ierr);
    VecAssemblyEnd(eta_vec);     CHKERRQ(ierr);
	...
	START_STD_LOOP
	{
		...
		// store current eta value in vector
        ierr = VecSetValue(eta_vec,iter,eta,INSERT_VALUES);   CHKERRQ(ierr);


       // PetscReal t = 1.23243454352345;
       // PetscViewer viewert;
       // PetscRealView(1, &t, PETSC_VIEWER_STDOUT_SELF);

		iter++;

		// get mesh steps (distance btw nodes)
		dx = SIZE_CELL(i, sx, fs->dsx);
		dy = SIZE_CELL(j, sy, fs->dsy);
		dz = SIZE_CELL(k, sz, fs->dsz);

		// get mesh steps for the backward and forward derivatives (distance btw cell centers)
		...
		pm->getStiffMat(eta, diag, v, cf, dx, dy, dz, fdx, fdy, fdz, bdx, bdy, bdz, v_fix);
		...
		constrLocalMat(6, pdofidx, cf, v_fix);
        ...
        // dump decompusitioning matrices
	    PetscViewer viewer;
	    PetscBool   flg;
	    char        iter_ch[20],name[50],

	    ierr = PetscOptionsHasName(NULL, NULL, "-dump_decomposition_matrices", &flg); CHKERRQ(ierr);

        if (flg)
	    { 
		    Mat Vis;
	        ierr = MatCreateSeqAIJ(PETSC_COMM_SELF, n_tot, n_tot, 100, NULL, &Vis);                 CHKERRQ(ierr);
            ierr = MatZeroEntries(Vis);                                                            CHKERRQ(ierr);

	    	ierr = MatSetValues(Vis, 6, idx, 6, idx, v_fix, INSERT_VALUES);    CHKERRQ(ierr);
		
            ierr = MatAssemblyBegin(Vis,MAT_FINAL_ASSEMBLY);     CHKERRQ(ierr);
            ierr = MatAssemblyEnd(Vis,MAT_FINAL_ASSEMBLY);       CHKERRQ(ierr);

	        sprintf(iter_ch, "%d", iter);
		    sprintf(name,"Matrices/Vis%s.bin",iter_ch);
		    PetscViewerBinaryOpen(PETSC_COMM_WORLD,name,FILE_MODE_WRITE,&viewer);
		    MatView(Vis, viewer);
		    PetscViewerDestroy(&viewer);
	   
            // destroy matrix again
	        ierr = MatDestroy(&Vis); CHKERRQ(ierr);
	    }
	...
	START_STD_LOOP
	{
        ...
        // store current eta value in vector
        ierr = VecSetValue(eta_vec,neta,eta,INSERT_VALUES);   CHKERRQ(ierr);
		neta ++;
        ...
        // =======================================================================================================
		v_fix[0]  =  1.0/dy/bdy; v_fix[1]  = -1.0/dy/bdy; v_fix[2]  =  1.0/dx/bdy; v_fix[3]  = -1.0/dx/bdy;
		v_fix[4]  = -1.0/dy/fdy; v_fix[5]  =  1.0/dy/fdy; v_fix[6]  = -1.0/dx/fdy; v_fix[7]  =  1.0/dx/fdy; 
		v_fix[8]  =  1.0/dy/bdx; v_fix[9]  = -1.0/dy/bdx; v_fix[10] =  1.0/dx/bdx; v_fix[11] = -1.0/dx/bdx; 
		v_fix[12] = -1.0/dy/fdx; v_fix[13] =  1.0/dy/fdx; v_fix[14] = -1.0/dx/fdx; v_fix[15] =  1.0/dx/fdx; 
        // =======================================================================================================
        ...
        if (flg)
	    {
            ...
            Mat Vis;
	        ierr = MatCreateSeqAIJ(PETSC_COMM_SELF, n_vis, n_vis, 100, NULL, &Vis);   CHKERRQ(ierr);
            ierr = MatZeroEntries(Vis);   

		    ierr = MatSetValues(Vis, 4, idx, 4, idx, v_fix, INSERT_VALUES);          CHKERRQ(ierr);

		    ierr = MatAssemblyBegin(Vis,MAT_FINAL_ASSEMBLY);                         CHKERRQ(ierr);
            ierr = MatAssemblyEnd(Vis,MAT_FINAL_ASSEMBLY);                           CHKERRQ(ierr);

		    sprintf(iter_ch, "%d",neta);
		    sprintf(name,"Matrices/Vis%s.bin",iter_ch);
		    PetscViewerBinaryOpen(PETSC_COMM_WORLD,name,FILE_MODE_WRITE,&viewer);
		    MatView(Vis, viewer);
		    PetscViewerDestroy(&viewer);
	...
	START_STD_LOOP
	{
        ...
        // =======================================================================================================
	    v_fix[0]  =  1.0/dz/bdz; v_fix[1]  = -1.0/dz/bdz; v_fix[2]  =  1.0/dx/bdz; v_fix[3]  = -1.0/dx/bdz; 
		v_fix[4]  = -1.0/dz/fdz; v_fix[5]  =  1.0/dz/fdz; v_fix[6]  = -1.0/dx/fdz; v_fix[7]  =  1.0/dx/fdz; 
		v_fix[8]  =  1.0/dz/bdx; v_fix[9]  = -1.0/dz/bdx; v_fix[10] =  1.0/dx/bdx; v_fix[11] = -1.0/dx/bdx;
		v_fix[12] = -1.0/dz/fdx; v_fix[13] =  1.0/dz/fdx; v_fix[14] = -1.0/dx/fdx; v_fix[15] =  1.0/dx/fdx; 
        // =======================================================================================================
	...
	START_STD_LOOP
	{
		...
		// store current eta value in vector
        ierr = VecSetValue(eta_vec,neta, eta,INSERT_VALUES);
		neta ++;
		...
		v[12] = -eta/dz/fdy; v[13] =  eta/dz/fdy; v[14] = -eta/dy/fdy; v[15] =  eta/dy/fdy; // fz_(j)   [syz]
		        ...
		        // =======================================================================================================
		v_fix[0]  =  1.0/dz/bdz; v_fix[1]  = -1.0/dz/bdz; v_fix[2]  =  1.0/dy/bdz; v_fix[3]  = -1.0/dy/bdz; // fx_(j-1) [sxy]
		v_fix[4]  = -1.0/dz/fdz; v_fix[5]  =  1.0/dz/fdz; v_fix[6]  = -1.0/dy/fdz; v_fix[7]  =  1.0/dy/fdz; // fx_(j)   [sxy]
		v_fix[8]  =  1.0/dz/bdy; v_fix[9]  = -1.0/dz/bdy; v_fix[10] =  1.0/dy/bdy; v_fix[11] = -1.0/dy/bdy; // fy_(i-1) [sxy]
		v_fix[12] = -1.0/dz/fdy; v_fix[13] =  1.0/dz/fdy; v_fix[14] = -1.0/dy/fdy; v_fix[15] =  1.0/dy/fdy; // fy_(i)   [sxy]
        // =======================================================================================================
    ...
    // ==============================================================================
    // restore bc constraints
	PetscInt  		numbc;
    numbc   = bc->vNumSPC;
	num = 0;
    Vec bc_vec;
	ierr = VecCreateSeq(PETSC_COMM_SELF,numbc,&bc_vec); CHKERRQ(ierr);
	ierr = VecSet(bc_vec,num);  CHKERRQ(ierr);
	VecAssemblyBegin(bc_vec);
    VecAssemblyEnd(bc_vec); 

	for(i = 0; i < numbc; i++) 
	{
		PetscScalar bc_row;
		bc_row = bc->SPCList[i];
		ierr = VecSetValue(bc_vec,i,bc_row,INSERT_VALUES); CHKERRQ(ierr);
	}
	// ==============================================================================


    VecAssemblyBegin(eta_vec);
    VecAssemblyEnd(eta_vec); 

	// dump preconditioning matrices to disk to inspect them with MATLAB (mainly for debugging)
	PetscViewer viewer;
	PetscBool   flg, flg_name;
	char        name[100], name_A[_str_len_], name_M[_str_len_], name_vec[20];
	...
	if (flg)
	{
		...
		PetscPrintf(PETSC_COMM_WORLD, "==== !!!!! ==== dump A matrix ==== !!!!! ====\n");
		sprintf(name_A,"Matrices/%s_A.bin",name);
		ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,name_A,FILE_MODE_WRITE,&viewer);  CHKERRQ(ierr);
		ierr = MatView(P->A, viewer);                                                   CHKERRQ(ierr);
		//PetscViewerDestroy(&viewer);

		// dump the M preconditioning matrix 2 disk
		sprintf(name_M,"Matrices/%s_M.bin",name);
		PetscViewerBinaryOpen(PETSC_COMM_WORLD,name_M,FILE_MODE_WRITE,&viewer);
		MatView(P->M, viewer);
		//PetscViewerDestroy(&viewer);


		// dump number of bc rows vector --> rows to zero out
		sprintf(name_vec,"Matrices/zrows.bin");
	   // ierr = PetscViewerPushFormat(viewer,PETSC_VIEWER_BINARY_MATLAB);   CHKERRQ(ierr);
		PetscViewerBinaryOpen(PETSC_COMM_WORLD,name_vec,FILE_MODE_WRITE,&viewer);
		VecView(bc_vec, viewer);
	    VecDestroy(&bc_vec);
		//ierr = PetscViewerPopFormat(viewer);   CHKERRQ(ierr);
        //PetscViewerDestroy(&viewer);

        // dump eta vector
		sprintf(name_vec,"Matrices/eta.bin");
	    //ierr = PetscViewerPushFormat(viewer,PETSC_VIEWER_ASCII_MATLAB);   CHKERRQ(ierr);
	    //ierr = PetscViewerPushFormat(viewer,PETSC_VIEWER_BINARY_MATLAB);   CHKERRQ(ierr);
		//PetscViewerASCIIOpen(PETSC_COMM_WORLD,name_vec,&viewer);
		PetscViewerBinaryOpen(PETSC_COMM_WORLD,name_vec,FILE_MODE_WRITE,&viewer);
		VecView(eta_vec, viewer);
	    VecDestroy(&eta_vec);
		//ierr = PetscViewerPopFormat(viewer);   CHKERRQ(ierr);
        PetscViewerDestroy(&viewer);

	}
       /* ierr = PetscOptionsHasName(NULL, NULL, "-only_matrix", &flg); CHKERRQ(ierr);
        if (flg)
	    {
...
PetscErrorCode PMatBlockAssemble(PMat pm)
{
	...
	PMatBlock   *P;
	PetscInt    idx[7];
	PetscScalar v[49], a[36], d[6], g[6], v_fix[49];
	PetscInt    mcx, mcy, mcz;
	PetscInt    iter, i, j, k, nx, ny, nz, sx, sy, sz;
	...
	
{
    ...
    //filling up v_fix
	//       vx_(i)               vx_(i+1)             vy_(j)               vy_(j+1)             vz_(k)               vz_(k+1)             p
	v_fix[0]  =  4.0/3.0/dx/bdx; v_fix[1]  = -4.0/3.0/dx/bdx; v_fix[2]  = -2.0/3.0/dy/bdx; v_fix[3]  =  2.0/3.0/dy/bdx; v_fix[4]  = -2.0/3.0/dz/bdx; v_fix[5]  =  2.0/3.0/dz/bdx;
	v_fix[6]  = -4.0/3.0/dx/fdx; v_fix[7]  =  4.0/3.0/dx/fdx; v_fix[8]  =  2.0/3.0/dy/fdx; v_fix[9]  = -2.0/3.0/dy/fdx; v_fix[10] =  2.0/3.0/dz/fdx; v_fix[11] = -2.0/3.0/dz/fdx;
	v_fix[12] = -2.0/3.0/dx/bdy; v_fix[13] =  2.0/3.0/dx/bdy; v_fix[14] =  4.0/3.0/dy/bdy; v_fix[15] = -4.0/3.0/dy/bdy; v_fix[16] = -2.0/3.0/dz/bdy; v_fix[17] =  2.0/3.0/dz/bdy;
	v_fix[18] =  2.0/3.0/dx/fdy; v_fix[19] = -2.0/3.0/dx/fdy; v_fix[20] = -4.0/3.0/dy/fdy; v_fix[21] =  4.0/3.0/dy/fdy; v_fix[22] =  2.0/3.0/dz/fdy; v_fix[23] = -2.0/3.0/dz/fdy;
	v_fix[24] = -2.0/3.0/dx/bdz; v_fix[25] =  2.0/3.0/dx/bdz; v_fix[26] = -2.0/3.0/dy/bdz; v_fix[27] =  2.0/3.0/dy/bdz; v_fix[28] =  4.0/3.0/dz/bdz; v_fix[29] = -4.0/3.0/dz/bdz;
	v_fix[30] =  2.0/3.0/dx/fdz; v_fix[31] = -2.0/3.0/dx/fdz; v_fix[32] =  2.0/3.0/dy/fdz; v_fix[33] = -2.0/3.0/dy/fdz; v_fix[34] = -4.0/3.0/dz/fdz; v_fix[35] =  4.0/3.0/dz/fdz;

	//filling up v_fix
	//       vx_(i)               vx_(i+1)             vy_(j)               vy_(j+1)             vz_(k)               vz_(k+1)             p
	/*v_fix[0]  =  4.0/3.0/dx/bdx; v_fix[1]  = -4.0/3.0/dx/bdx; v_fix[2]  = -2.0/3.0/dy/bdx; v_fix[3]  =  2.0/3.0/dy/bdx; v_fix[4]  = -2.0/3.0/dz/bdx; v_fix[5]  =  2.0/3.0/dz/bdx; v_fix[6]  =  1.0/bdx; // fx_(i)   [sxx]
	v_fix[7]  = -4.0/3.0/dx/fdx; v_fix[8]  =  4.0/3.0/dx/fdx; v_fix[9]  =  2.0/3.0/dy/fdx; v_fix[10] = -2.0/3.0/dy/fdx; v_fix[11] =  2.0/3.0/dz/fdx; v_fix[12] = -2.0/3.0/dz/fdx; v_fix[13] = -1.0/fdx; // fx_(i+1) [sxx]
	v_fix[14] = -2.0/3.0/dx/bdy; v_fix[15] =  2.0/3.0/dx/bdy; v_fix[16] =  4.0/3.0/dy/bdy; v_fix[17] = -4.0/3.0/dy/bdy; v_fix[18] = -2.0/3.0/dz/bdy; v_fix[19] =  2.0/3.0/dz/bdy; v_fix[20] =  1.0/bdy; // fy_(j)   [syy]
	v_fix[21] =  2.0/3.0/dx/fdy; v_fix[22] = -2.0/3.0/dx/fdy; v_fix[23] = -4.0/3.0/dy/fdy; v_fix[24] =  4.0/3.0/dy/fdy; v_fix[25] =  2.0/3.0/dz/fdy; v_fix[26] = -2.0/3.0/dz/fdy; v_fix[27] = -1.0/fdy; // fy_(j+1) [syy]
	v_fix[28] = -2.0/3.0/dx/bdz; v_fix[29] =  2.0/3.0/dx/bdz; v_fix[30] = -2.0/3.0/dy/bdz; v_fix[31] =  2.0/3.0/dy/bdz; v_fix[32] =  4.0/3.0/dz/bdz; v_fix[33] = -4.0/3.0/dz/bdz; v_fix[34] =  1.0/bdz; // fz_(k)   [szz]
	v_fix[35] =  2.0/3.0/dx/fdz; v_fix[36] = -2.0/3.0/dx/fdz; v_fix[37] =  2.0/3.0/dy/fdz; v_fix[38] = -2.0/3.0/dy/fdz; v_fix[39] = -4.0/3.0/dz/fdz; v_fix[40] =  4.0/3.0/dz/fdz; v_fix[41] = -1.0/fdz; // fz_(k+1) [szz]
	v_fix[42] =  1.0/dx;     v_fix[43] = -1.0/dx;     v_fix[44] =  1.0/dy;     v_fix[45] = -1.0/dy;     v_fix[46] =  1.0/dz;     v_fix[47] = -1.0/dz;     v_fix[48] =  1.0;     // g
*/
}
//---------------------------------------------------------------------------
	...
	PetscScalar *v_fix)
{
	...
	//filling up v_fix
	//       vx_(i)               vx_(i+1)             vy_(j)               vy_(j+1)             vz_(k)               vz_(k+1)             p
	v_fix[0]  =  2.0/dx/bdx; v_fix[1]  = -2.0/dx/bdx; v_fix[2]  =  0.0       ; v_fix[3]  =  0.0       ; v_fix[4]  =  0.0       ; v_fix[5]  =  0.0       ; v_fix[6]  =  1.0/bdx; // fx_(i)   [sxx]
	v_fix[7]  = -2.0/dx/fdx; v_fix[8]  =  2.0/dx/fdx; v_fix[9]  =  0.0       ; v_fix[10] =  0.0       ; v_fix[11] =  0.0       ; v_fix[12] =  0.0       ; v_fix[13] = -1.0/fdx; // fx_(i+1) [sxx]
	v_fix[14] =  0.0       ; v_fix[15] =  0.0       ; v_fix[16] =  2.0/dy/bdy; v_fix[17] = -2.0/dy/bdy; v_fix[18] =  0.0       ; v_fix[19] =  0.0       ; v_fix[20] =  1.0/bdy; // fy_(j)   [syy]
	v_fix[21] =  0.0       ; v_fix[22] =  0.0       ; v_fix[23] = -2.0/dy/fdy; v_fix[24] =  2.0/dy/fdy; v_fix[25] =  0.0       ; v_fix[26] =  0.0       ; v_fix[27] = -1.0/fdy; // fy_(j+1) [syy]
	v_fix[28] =  0.0       ; v_fix[29] =  0.0       ; v_fix[30] =  0.0       ; v_fix[31] =  0.0       ; v_fix[32] =  2.0/dz/bdz; v_fix[33] = -2.0/dz/bdz; v_fix[34] =  1.0/bdz; // fz_(k)   [szz]
	v_fix[35] =  0.0       ; v_fix[36] =  0.0       ; v_fix[37] =  0.0       ; v_fix[38] =  0.0       ; v_fix[39] = -2.0/dz/fdz; v_fix[40] =  2.0/dz/fdz; v_fix[41] = -1.0/fdz; // fz_(k+1) [szz]
	v_fix[42] =  1.0/dx;     v_fix[43] = -1.0/dx;     v_fix[44] =  1.0/dy;     v_fix[45] = -1.0/dy;     v_fix[46] =  1.0/dz;     v_fix[47] = -1.0/dz;     v_fix[48] =  1.0;     // g
}
//---------------------------------------------------------------------------
